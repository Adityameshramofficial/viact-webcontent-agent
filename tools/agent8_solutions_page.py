...
_FAQ_SCHEMA = """{
  "faq_1_q": "Question?", "faq_1_a": "Answer (2-4 sentences).",
  "faq_2_q": "...", "faq_2_a": "...",
  "faq_3_q": "...", "faq_3_a": "...",
  "faq_4_q": "...", "faq_4_a": "...",
  "faq_5_q": "...", "faq_5_a": "...",
...
def _build_faq_prompt(solution_name: str, cms: dict) -> list:
    ...
    system = (
        ...
        "Each answer: 2-5 sentences, include viAct product name. "
        "Output ONLY valid JSON."
    )
    user = (
        f"Generate 10 FAQ pairs for viAct's {solution_name} page.\n"
        f"Product: {desc}\nKey features: {tabs}\n\n"
        "Cover: (1) What is it, (2) How it works, (3) Industries, (4) Integration, "
        "(5) AI/CV capabilities, (6) ROI/outcomes, (7) Compliance standards, "
        "(8) Deployment (Edge/cloud), (9) Customization, (10) vs traditional methods.\n\n"
        f"JSON SCHEMA:\n{_FAQ_SCHEMA}"
    )
    return [{"role": "system", "content": system}, {"role": "user", "content": user}]
...
def generate_solutions_page(
    ...
    if run_tavily:
        emit(f"Step 1/4 — Fetching live research for '{solution_name}'...")
        research = _tavily_research(solution_name)
    else:
        emit("Step 1/4 — Skipping Tavily research.")
        research = ""

    emit("Step 2/4 — Fetching viAct style reference...")
    style_ref = _scrape_viact_reference()

    emit("Step 3/4 — Generating main content (Llama 3.3 70B)...")
