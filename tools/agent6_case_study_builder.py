...
{{
  "hero_h1": "Achievement-focused headline, e.g. '[Company] Reduces Safety Incidents by 80% with viAct AI'",
  ...
  "challenge_body": "3-4 paragraphs (200-300 words). The Challenge section: specific pain points, manual processes, safety risks, compliance gaps, operational impact. Include H3-level sub-context.",
  "solution_body": "3-4 paragraphs (200-300 words). The Solution section: how viAct was deployed, specific modules used, what changed operationally. Include H3 sub-points per product/feature.",
  "impact_body": "2-3 paragraphs (120-180 words). The Impact section: quantified results first, then qualitative improvements, long-term value.",
  "metric_1_value": "primary metric value e.g. '10x' or '80%'",
  "metric_1_label": "primary metric label e.g. 'Productivity Improvement'",
  ...
  "testimonial_1_quote": "Direct quote (1-3 sentences) from a safety/EHS professional at the company. Specific benefit, not generic praise.",
  "testimonial_1_role": "Role title e.g. 'EHS Director, {company}'",
  "testimonial_2_quote": "Second quote from a different role (project manager, safety supervisor, etc.)",
  "testimonial_2_role": "Role title e.g. 'Project Safety Manager, {company}'",
  "cta_headline": "Bring the same results to your {industry.lower()} site",
  "meta_title": "SEO title max 60 chars e.g. '{company} Case Study | viAct AI Safety'",
  "meta_description": "SEO description 150-160 chars",
  "slug": "url-slug e.g. '{company.lower().replace(' ', '-')}-viact-case-study'",
  "hero_image_brief": "Nano Banana style image prompt: 35-50 words, APAC industrial setting, realistic, safety context, 1200x630px"
}}

Rules:
- challenge_body, solution_body, impact_body: use plain text (no markdown headers inside)
- All metric values: use real numbers from references if provided, else use [bracketed placeholders]
...
def _validate(content: dict) -> list[str]:
    ...
    for field in ["challenge_body", "solution_body", "impact_body"]:
        val = content.get(field, "")
        if len(val.split()) < 50:
            errors.append(f"{field}: too short ({len(val.split())} words, need ≥50)")
